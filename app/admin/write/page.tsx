...
// useSearchParams를 담은 내부 컴포넌트 분리
function WriteForm() {
      ...
      <form action={createPost} className="flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-16">
          <div className="flex flex-col gap-5">
            ...
            <button
              type="button"
              className="w-full flex items-center justify-center bg-point/40 border border-text-point/20 p-5 rounded-main hover:bg-point hover:border-text-point/70 text-20 hover:text-20-bold text-text-2"
              onClick={addSection}>
              <span>추가하기</span>
            </button>
          </div>
          ...
          <div className="flex flex-col gap-5">
            <input
              ...
              type="password"
              placeholder="관리자 암호"
              className="p-4 rounded flex items-center justify-center bg-point/40 border border-text-point/20 hover:bg-point hover:border-text-point/70 text-20 hover:text-20-bold text-text-2"
              required
            />
            <button
              type="submit"
              className="p-5 rounded-main bg-bg-component border border-text-point/50 hover:shadow-default text-20 hover:text-20-bold">
              포스트 게시
            </button>
          </div>
        </div>
...
// 메인 페이지 컴포넌트: useSearchParams를 쓰는 WriteForm을 완전히 Suspense로 감싸 바운더리 형성
export default function WritePage() {
  return (
    <Suspense fallback={<div className="max-w-281 mx-auto py-22 text-center">로딩중...</div>}>
      <WriteForm />
    </Suspense>
  );
}
